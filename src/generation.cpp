...
void Generate_Thread(tilemap& map, int startY, int lines, int seed)
{
    ...
    fnFractal->GenUniformGrid2D(basicNoiseOutput.data(), 0, startY, map.GetWidth(), lines, 0.05f, seed);
    ...
    fnCaveFractal->SetSource(fnPerlin);
    fnCaveFractal->SetOctaveCount(4);
    fnCaveFractal->GenUniformGrid2D(caveNoiseOutput.data(), 0, startY, map.GetWidth(), lines, 0.05f, seed + 2);
    ...
    for (u64 i = startPos; i < startPos + lines * map.GetWidth(); i++)
    {
        ...
        f32 surfaceRigidness = 0.01f; // affects the rigidness of the surface
        ...
        f32 temperatureMapScale = 0.2f; // affects how much the temp map affects the final temperature
        ...
        f32 caveThreshold = 0.4f; // affects how big caves are
        ...
        if (depthGradient < groundStartThreshold)
        {
            ...
            if (caveNoiseOutput[localIndex] + depthGradient * caveDepthScale + cavePerturb > caveThreshold)
            {
