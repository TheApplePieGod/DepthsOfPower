...
{
    input_manager& manager = Engine->GetInputManager();
    if (action == GLFW_PRESS)
        manager.SetKeyPressed(key, true);
    else if (action == GLFW_RELEASE)
        manager.SetKeyPressed(key, false);
}
...
void mouse_button_callback(GLFWwindow* window, int button, int action, int mods)
{
    input_manager& manager = Engine->GetInputManager();
    if (action == GLFW_PRESS)
        manager.SetMouseButtonPressed(button + 1, true);
    else if (action == GLFW_RELEASE)
        manager.SetMouseButtonPressed(button + 1, false);
}

void input_manager::Initialize()
{
    glfwSetKeyCallback(Engine->GetRenderer().Window(), key_callback);
    glfwSetMouseButtonCallback(Engine->GetRenderer().Window(), mouse_button_callback);
}
...
{
    double xpos, ypos;
    glfwGetCursorPos(Engine->GetRenderer().Window(), &xpos, &ypos);
